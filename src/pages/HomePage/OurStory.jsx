...
const storyData = {
  ...
  part1: {
    heading: "Our Story",
    leftParagraph: "In a world that moves faster every day, small gifts made with care have become more meaningful than we often realize. And that... is the inspiration behind Cottonclix.",
    rightParagraph: "Cottonclix was born from a love for fabric, a passion for nature-inspired patterns, and the belief that “a good gift doesn't have to be big , it just has to be meaningful.”",
  },
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-8 ">
                    ...
                    <div className="md:col-span-2 text-left ">
                        <div className=" grid grid-cols-1 md:grid-cols-1 items-center justify-end ">
                            <img src={cottonFlower} alt="Cotton Flower Divider" className="absolute w-[25%] right-[40%] h-auto opacity-100 z-0 pointer-events-none drop-shadow-2xl" />
                            <div className='grid grid-cols-1 md:grid-cols-1 items-center justify-end'>
                                <div className="flex items-center justify-end gap-4 ">
                                    <img src={logo} alt="Cottonclix Logo" className="h-32 w-auto " />
                                </div>
                ...
                <div className="relative grid grid-cols-2 md:grid-cols-1 gap-x-16 gap-y-8 text-lg text-[#8D8D8D] leading-relaxed">
                    <div className="grid grid-cols-2 md:grid-cols-2 gap-x-16 gap-y-8 text-lg text-[#8D8D8D] leading-relaxed">
                        ...
                        <div className="space-y-6">
                            {storyData.bottomSection.leftParagraphs.map((text, i) => <p key={i}>{text}</p>)}
                        </div>

                        {/* คอลัมน์ขวา */}
                        <div className="relative ">
                            <img src={lineArt} alt="Line Art" className="absolute  -top-[45%] scale-y-[-1] right-0 w-48 h-auto opacity-100 hidden md:block rotate-90 drop-shadow-2xl" />
                            <div className="space-y-2 text-lg text-[#111000] font-medium">
                                {storyData.bottomSection.rightList.map((item, i) => <p key={i}>{item}</p>)}
                            </div>
                            
                            
                            {storyData.bottomSection.rightParagraphs.map((text, i) => <p key={i}>{text}</p>)}
                    ...
                    <div className=" flex justify-center items-center gap-4 ">
                        <img src={cottonFlower} alt="Cotton Flower" className="w-[25%] h-auto absolute left-[20%]  opacity-100 hidden md:block drop-shadow-2xl" />
                        <div>
                            
                            <p className="font-serif font-semibold text-[#BC9F31] text-3xl pt-4">{storyData.bottomSection.quote}</p>
